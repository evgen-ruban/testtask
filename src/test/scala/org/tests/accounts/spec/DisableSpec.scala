...
class DisableSpec extends FlatSpec with ScalatestRouteTest with HttpService with AccountRoute with ScalaFutures with Matchers with MockitoSugar with CommonResults {
  val url = "/disable"
  ...
  "Account" should "not be disable with 'Error' message" in {
    val id = BSONObjectID.generate.stringify
    val account = accountFactory(Some(id))

    when(accountRepository.findAccountById(Some(id))).thenReturn(someResult(account))

    when(accountRepository.disableAccount(account)).thenReturn(exceptionResult)
